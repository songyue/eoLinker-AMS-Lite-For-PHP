    ...
    angular.module('eolinker')
        ...
        function indexController(ERROR_WARNING, $scope, ApiManagementResource, $state, CODE, $rootScope, GroupService, Group_AmsCommonService) {
            ...
            vm.data = {
                ...
                static: {
                    query: [{ groupID: -1, groupName: "所有用例", icon: 'sort' }]
                },
                component: {
                    groupCommonObject: {}
                },
                ...
                interaction: {
                    request: {
                        companyHashKey: $state.params.companyHashKey,
            ...
            vm.data.fun.init = function() {
                ...
                if (/test.((api)|(edit)|(mode))/.test($state.current.name)) {
            ...
            vm.data.fun.sort.confirm = function() {
                var template = {
                    input: {
                        baseRequest: {
                            companyHashKey: vm.data.interaction.request.companyHashKey,
                            projectID: vm.data.interaction.request.projectID,
                            orderNumber: {}
                        },
                                ...
                                case CODE.COMMON.SUCCESS:
                                    {
                                        ...
                                        switch((vm.data.interaction.request.groupID||0).toString()){
                                            case '-1':{
                                                $state.reload('home.project.inside.test.default');
                                                break;
                                            }
                                            default:{
            ...
            $scope.$on('$stateChangeSuccess', function() { //路由更改函数
                if (/(api)|(edit)|(mode)/.test($state.current.name)) {
                    vm.data.info.sidebarShow = false;
                } else {
                    vm.data.info.sidebarShow = true;
                }
