(function() {
    ...
    angular.module('eolinker')
        .component('homeProjectInsideDocSidebar', {
            templateUrl: 'app/component/content/home/content/project/content/inside/sidebar/doc/index.html',
            bindings: {
                powerObject: '<'
            },
    ...
    function indexController($scope, ApiManagementResource, $state, CODE, $rootScope, GroupService, Sidebar_AmsCommonService, $filter) {
        ...
        vm.data = {
            ...
            static: {
                query: [{ groupID: -1, groupName: $filter('translate')('01214023'), icon: 'sort' }]
            },
            component: {
                groupCommonObject: {}
            },
        ...
        vm.data.fun.init = function() {
            ...
            ApiManagementResource.DocGroup.Query(template.request).$promise.then(function(response) {
        ...
        vm.data.fun.sort.confirm = function() {
            var template = {
                input: {
                    ...
                    resource: ApiManagementResource.DocGroup.Sort,
                    callback: null
                }
            }
        ...
        vm.data.fun.click.child = function(arg) {
            vm.data.interaction.request.childGroupID = arg.item.groupID || -1;
            $state.go('home.project.inside.doc.list', { groupID: vm.data.interaction.request.groupID, childGroupID: arg.item.groupID, apiID: null, search: null });
        }
        /**
         * @function [父分组单击事件] [Parent group click event]
         */
        vm.data.fun.click.parent = function(arg) {
            vm.data.interaction.request.groupID = arg.item.groupID;
            vm.data.interaction.request.childGroupID = null;
            arg.item.isSpreed = true;
            $state.go('home.project.inside.doc.list', { 'groupID': arg.item.groupID, childGroupID: null, search: null });
        }
        /**
        ...
        vm.data.fun.edit.parent = function(arg) {
            ...
            $rootScope.GroupModal(template.modal.title, arg.item, template.modal.secondTitle, template.modal.group, function(callback) {
                if (callback) {
                    ...
                    if (arg.item) {
                        ApiManagementResource.DocGroup.Update(callback).$promise.then(function(response) {
                            switch (response.statusCode) {
                                case CODE.COMMON.SUCCESS:
                                    {
                        ...
                        ApiManagementResource.DocGroup.Add({ projectID: callback.projectID, groupName: callback.groupName, parentGroupID: callback.parentGroupID }).$promise.then(function(response) {
                            switch (response.statusCode) {
                                case CODE.COMMON.SUCCESS:
                                    {
                                        $rootScope.InfoModal(template.modal.title + $filter('translate')('01214015'), 'success');
            ...
            $rootScope.GroupModal(template.modal.title, arg.childItem, $filter('translate')('01214014'), template.modal.group, function(callback) {
                if (callback) {
                    ...
                    if (arg.isEdit) {
                        ...
                        ApiManagementResource.DocGroup.Add({ parentGroupID: callback.parentGroupID, projectID: vm.data.interaction.request.projectID, groupName: callback.groupName }).$promise.then(function(response) {
                            switch (response.statusCode) {
                                case CODE.COMMON.SUCCESS:
                                    {
                                        $rootScope.InfoModal(template.modal.title + $filter('translate')('01214015'), 'success');
        ...
        vm.data.fun.delete.child = function(arg) {
            ...
            $rootScope.EnsureModal(template.modal.title, false, template.modal.message, {}, function(callback) {
                if (callback) {
                    ApiManagementResource.DocGroup.Delete({ projectID: vm.data.interaction.request.projectID, groupID: arg.childItem.groupID }).$promise.then(function(response) {
                        switch (response.statusCode) {
                            case CODE.COMMON.SUCCESS:
                                {
        ...
        vm.data.fun.delete.parent = function(arg) {
            ...
            $rootScope.EnsureModal(template.modal.title, false, template.modal.message, {}, function(callback) {
                if (callback) {
                    ApiManagementResource.DocGroup.Delete({ projectID: vm.data.interaction.request.projectID, groupID: arg.item.groupID }).$promise.then(function(response) {
                        switch (response.statusCode) {
                            case CODE.COMMON.SUCCESS:
                                {
                                    ...
                                    } else if (vm.data.interaction.request.groupID == arg.item.groupID || !$state.params.groupID) {
